...
public class ProductController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		case "/modify.pd":
			...
			request.setAttribute("pvo", svc.detail(pno1));
			System.out.println(">>> 상품수정 성공~!!");
			destPage = "/modify.jsp";
			break;
		case "/edit.pd":
			String pnoParam = request.getParameter("pno");
			int pno2 = Integer.parseInt(pnoParam);
			String newPname = request.getParameter("pname");
			int newPrice = Integer.parseInt(request.getParameter("price"));
			String newMadeby = request.getParameter("madeby");
			ProductVO p2 = new ProductVO(pno2, newPname, newPrice, newMadeby);
			svc.edit(p2);
			System.out.println(">>> 상품수정 성공~!!");
			destPage = "/list.pd";
			break;
