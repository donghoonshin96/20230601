...
//			int updateResult = bsv.updateBoard(updatedBoard);
		...
		case "delete":
		    ...
		    // 게시글 정보 가져오기
		    BoardVO deletedBoard = bsv.getBoard(deletedBno);

		    // 게시글에 첨부된 이미지 파일 삭제
		    String deletedImageFile = deletedBoard.getImage_file();
		    if (deletedImageFile != null && !deletedImageFile.isEmpty()) {
		        String savePath = getServletContext().getRealPath("/_fileUpload");
		        FileHandler fileHandler = new FileHandler();
		        int deleteResult = fileHandler.deleteFile(deletedImageFile, savePath);
		        if (deleteResult != 0) {
		            log.info(">>> Deleted image file: " + deletedImageFile);
		        } else {
		            log.info(">>> Failed to delete image file: " + deletedImageFile);
		        }
